...
async function connectWithToken(token) {
    const url = 'https://api.distribute.ai/internal/auth/connect';
...
async function getAllProviders(token) {
    const url = 'https://api.distribute.ai/internal/provider/providers?limit=100';
...
async function deleteProviders(token, nodeId) {
    const url = `https://api.distribute.ai/internal/provider/?id=${nodeId}`;
    const headers = {
        'Authorization': token,
    };
